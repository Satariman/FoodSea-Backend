...
// alongside the underlying *sql.DB (needed for raw-SQL repositories such as search).
// It configures the connection pool and verifies the connection with a Ping.
...
func maskURL(url string) string {
	...
	// Full URL form: postgres://user:pass@host/db
	for i := 0; i < len(url); i++ {
		if url[i] == ':' && i+1 < len(url) {
			// Find @ sign to mask password
