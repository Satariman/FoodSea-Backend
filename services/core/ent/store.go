...
type StoreEdges struct {
	...
	// DeliveryCondition holds the value of the delivery_condition edge.
	DeliveryCondition *DeliveryCondition `json:"delivery_condition,omitempty"`
...
func (e StoreEdges) OffersOrErr() ([]*Offer, error) {
	if e.loadedTypes[0] {
		return e.Offers, nil
	}
...
// DeliveryConditionOrErr returns the DeliveryCondition value or an error if the edge
// was not loaded in eager-loading, or loaded but was not found.
func (e StoreEdges) DeliveryConditionOrErr() (*DeliveryCondition, error) {
	if e.DeliveryCondition != nil {
		return e.DeliveryCondition, nil
	} else if e.loadedTypes[1] {
		return nil, &NotFoundError{label: deliverycondition.Label}
	}
	return nil, &NotLoadedError{edge: "delivery_condition"}
}
...
func (*Store) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case store.FieldName, store.FieldSlug, store.FieldLogoURL:
...
// QueryOffers queries the "offers" edge of the Store entity.
func (_m *Store) QueryOffers() *OfferQuery {
	return NewStoreClient(_m.config).QueryOffers(_m)
}

// QueryDeliveryCondition queries the "delivery_condition" edge of the Store entity.
func (_m *Store) QueryDeliveryCondition() *DeliveryConditionQuery {
	return NewStoreClient(_m.config).QueryDeliveryCondition(_m)
}
