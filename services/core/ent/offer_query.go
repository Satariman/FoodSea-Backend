...
func (_q *OfferQuery) QueryProduct() *ProductQuery {
...
func (_q *OfferQuery) QueryStore() *StoreQuery {
...
func (_q *OfferQuery) WithProduct(opts ...func(*ProductQuery)) *OfferQuery {
	query := (&ProductClient{config: _q.config}).Query()
...
func (_q *OfferQuery) WithStore(opts ...func(*StoreQuery)) *OfferQuery {
	query := (&StoreClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OfferQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Offer, error) {
	...
	if query := _q.withProduct; query != nil {
		if err := _q.loadProduct(ctx, query, nodes, nil,
			func(n *Offer, e *Product) { n.Edges.Product = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withStore; query != nil {
		if err := _q.loadStore(ctx, query, nodes, nil,
			func(n *Offer, e *Store) { n.Edges.Store = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *OfferQuery) loadProduct(ctx context.Context, query *ProductQuery, nodes []*Offer, init func(*Offer), assign func(*Offer, *Product)) error {
...
func (_q *OfferQuery) loadStore(ctx context.Context, query *StoreQuery, nodes []*Offer, init func(*Offer), assign func(*Offer, *Store)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Offer)
	for i := range nodes {
		fk := nodes[i].StoreID
...
func (_q *OfferQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withProduct != nil {
			_spec.Node.AddColumnOnce(offer.FieldProductID)
		}
		if _q.withStore != nil {
			_spec.Node.AddColumnOnce(offer.FieldStoreID)
		}
	}
