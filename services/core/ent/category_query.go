...
type CategoryQuery struct {
	...
	withSubcategoryProducts *ProductQuery
...
// QuerySubcategoryProducts chains the current query on the "subcategory_products" edge.
func (_q *CategoryQuery) QuerySubcategoryProducts() *ProductQuery {
	query := (&ProductClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *CategoryQuery) Clone() *CategoryQuery {
	...
	return &CategoryQuery{
		...
		withSubcategoryProducts: _q.withSubcategoryProducts.Clone(),
...
// WithSubcategoryProducts tells the query-builder to eager-load the nodes that are connected to
// the "subcategory_products" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *CategoryQuery) WithSubcategoryProducts(opts ...func(*ProductQuery)) *CategoryQuery {
	query := (&ProductClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSubcategoryProducts = query
	return _q
}
...
func (_q *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	var (
		...
		loadedTypes = [4]bool{
			...
			_q.withParent != nil,
			_q.withProducts != nil,
			_q.withSubcategoryProducts != nil,
		}
	)
	...
	if query := _q.withSubcategoryProducts; query != nil {
		if err := _q.loadSubcategoryProducts(ctx, query, nodes,
			func(n *Category) { n.Edges.SubcategoryProducts = []*Product{} },
			func(n *Category, e *Product) { n.Edges.SubcategoryProducts = append(n.Edges.SubcategoryProducts, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CategoryQuery) loadSubcategoryProducts(ctx context.Context, query *ProductQuery, nodes []*Category, init func(*Category), assign func(*Category, *Product)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Category)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(product.FieldSubcategoryID)
	}
	query.Where(predicate.Product(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.SubcategoryProductsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.SubcategoryID
		if fk == nil {
			return fmt.Errorf(`foreign-key "subcategory_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "subcategory_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
