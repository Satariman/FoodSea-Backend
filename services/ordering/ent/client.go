...
import (
	...
	"github.com/foodsea/ordering/ent/migrate"
...
func (c *Client) Use(hooks ...Hook) {
	...
	c.OrderStatusHistory.Use(hooks...)
	c.SagaState.Use(hooks...)
}
...
func (c *Client) Intercept(interceptors ...Interceptor) {
	...
	c.OrderStatusHistory.Intercept(interceptors...)
	c.SagaState.Intercept(interceptors...)
}
...
// QueryHistory queries the history edge of a Order.
func (c *OrderClient) QueryHistory(_m *Order) *OrderStatusHistoryQuery {
	...
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, order.HistoryTable, order.HistoryColumn),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
...
// UpdateOneID returns an update builder for the given id.
func (c *SagaStateClient) UpdateOneID(id uuid.UUID) *SagaStateUpdateOne {
...
// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *SagaStateClient) DeleteOneID(id uuid.UUID) *SagaStateDeleteOne {
...
func (c *SagaStateClient) Get(ctx context.Context, id uuid.UUID) (*SagaState, error) {
	return c.Query().Where(sagastate.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *SagaStateClient) GetX(ctx context.Context, id uuid.UUID) *SagaState {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
func (c *SagaStateClient) Hooks() []Hook {
	hooks := c.hooks.SagaState
	return append(hooks[:len(hooks):len(hooks)], sagastate.Hooks[:]...)
}
...
type (
	hooks struct {
		Order, OrderItem, OrderStatusHistory, SagaState []ent.Hook
	}
	inters struct {
		Order, OrderItem, OrderStatusHistory, SagaState []ent.Interceptor
	}
)
