...
type OrderQuery struct {
	...
	withHistory *OrderStatusHistoryQuery
...
func (_q *OrderQuery) QueryHistory() *OrderStatusHistoryQuery {
...
func (_q *OrderQuery) WithHistory(opts ...func(*OrderStatusHistoryQuery)) *OrderQuery {
	query := (&OrderStatusHistoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := _q.withHistory; query != nil {
		if err := _q.loadHistory(ctx, query, nodes,
			func(n *Order) { n.Edges.History = []*OrderStatusHistory{} },
			func(n *Order, e *OrderStatusHistory) { n.Edges.History = append(n.Edges.History, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *OrderQuery) loadHistory(ctx context.Context, query *OrderStatusHistoryQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderStatusHistory)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Order)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.OrderStatusHistory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(order.HistoryColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
