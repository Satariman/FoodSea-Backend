...
// Config holds the full runtime configuration for optimization-service.
type Config struct {
	Env                 string
	Server              ServerConfig
...
func Load() (*Config, error) {
	...
	cfg := &Config{
		...
		GRPCClients: GRPCClientConfig{
			...
			MLAddr:   getEnv("ML_GRPC_ADDR", "ml-service:50051"),
		},
		OptimizationTimeout: getEnvDuration("OPTIMIZATION_TIMEOUT", 30*time.Second),
		ResultTTL:           getEnvDuration("RESULT_TTL", 30*time.Minute),
	}
