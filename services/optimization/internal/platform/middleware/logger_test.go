...
func TestLogger_2xx_IsInfo(t *testing.T) {
	...
	r.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/ok", http.NoBody))
...
func TestLogger_4xx_IsWarn(t *testing.T) {
	...
	w := httptest.NewRecorder()
	r.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/bad", http.NoBody))
...
func TestLogger_5xx_IsError(t *testing.T) {
	...
	w := httptest.NewRecorder()
	r.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/err", http.NoBody))
